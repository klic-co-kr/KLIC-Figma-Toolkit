...
3. **A+B 일괄**: 사용자가 "일괄 수리" 클릭 → `command-apply-fixes {tier:'AB'}` → 엔진이 모든 A+B provider 적용 후 **`figma.commitUndo()` 1회 호출** → 전체가 단일 undo 단위가 되어 한 번의 Ctrl+Z로 롤백. KLIC 변경 로그(인스펙터 undo log)에 요약 기록
4. **C 항목별**: 각 항목 카드에 before/after·델타 프리뷰 + 개별 "적용" 버튼 → `command-apply-fixes {ids:[id]}`. 항목 적용 직후 `figma.commitUndo()` 호출 → 항목별 독립 undo 단위

### Undo API 제약 (context7 검증)

> **중요**: Figma는 기본적으로 플러그인 변경을 undo 히스토리에 **넣지 않는다**. `figma.commitUndo()`를 명시 호출해야 사용자의 Ctrl+Z가 해당 변경을 되돌릴 수 있다. (출처: developers.figma.com/docs/plugins/api/properties/figma-commitundo)

- 엔진은 적용 경로 끝에서 반드시 `figma.commitUndo()`를 호출한다. 누락 시 사용자가 수리를 되돌릴 수 없다.
- A+B 일괄: 모든 변경 후 **마지막에 단 한 번** commitUndo → 1 undo 단위
- C 개별: 각 항목 적용 후 commitUndo → 항목당 1 undo 단위

복구 모델: **프리뷰 + `commitUndo` 기반 네이티브 undo.** 적용 전 집계 프리뷰로 확인, 적용 후 commitUndo가 만든 체크포인트를 Ctrl+Z로 롤백. 별도 영속 revert 저장은 하지 않는다.

### Figma API 패턴 제약 (context7 검증)
- **paint 바인딩**: fills는 불변 배열. `var copy = clone(node.fills); copy[i] = figma.variables.setBoundVariableForPaint(copy[i], 'color', variable); node.fills = copy;` 패턴 필수. 기존 `applyColorBindings`가 이미 사용 — 신규 paint 계열 fix도 동일.
...
## 10. 알려진 리스크 (구현 전 spike 필요)

context7 적대적 검토에서 문서로 확정하지 못한 항목 — 실 Figma에서 작은 spike로 검증 후 해당 provider 구현.

| 리스크 | 내용 | 완화 |
|---|---|---|
| **변수 삭제 시 바인딩 노드** | `consolidateDuplicateToken`이 중복 `Variable.remove()` 호출. 여전히 바인딩된 노드가 있으면 동작 미확인 (문서 미노출) | 삭제 전 해당 변수에 바인딩된 **모든 노드를 정규 토큰으로 재바인딩 완료**를 보장하는 순서 강제 + spike로 remove 후 잔여 바인딩 거동 확인. 불확실하면 이 provider를 C(항목별)로 강등 |
| **variant 추가 복잡도** | `addFocusState`가 기존 컴포넌트셋에 포커스 상태 variant 추가. `addComponentProperty`는 확인됐으나 variant 노드 삽입 전체 흐름은 미확인 | 최고 리스크 provider. 별도 spike. spike 실패 시 v1에서 제외하고 "포커스 상태 누락 N건 — 수동 처리" 안내로 대체 |
| **commitUndo 누락** | 호출 안 하면 사용자가 수리를 undo 불가 (검증 완료, §6 반영) | 엔진 적용 경로 끝 commitUndo 강제 + 회귀 테스트로 호출 검증 |
