figma.showUI(__html__, { width: 460, height: 760, title: '스타일 가이드 변수 생성기' });

figma.ui.onmessage = async function(msg) {
  if (msg.type === 'create-variables') {
    await createVariables(msg.data);
  } else if (msg.type === 'draw-style-guide') {
    await drawStyleGuide(msg.data);
  } else if (msg.type === 'create-components') {
    await createComponents(msg.data);
  } else if (msg.type === 'search-fonts') {
    await searchFonts(msg.query);
...
async function searchFonts(query) {
  try {
    ...
    results.sort();
    figma.ui.postMessage({ type: 'font-search-result', families: results.slice(0, 40) });
  } catch(e) {
    figma.ui.postMessage({ type: 'font-search-result', families: [] });
  }
}
...
function setColorVar(collection, name, hex) {
...
function setFloatVar(collection, name, value) {
  var modeId = collection.defaultModeId;
  var existing = figma.variables.getLocalVariables()
    .find(function(v) { return v.name === name && v.variableCollectionId === collection.id; });
...
async function createVariables(data) {
  ...
  function report(name) {
    ...
    figma.ui.postMessage({ type: 'progress', current: done, total: total, name: name });
  }

  try {
    var brandCol = getOrCreateCollection('컬러/브랜드');
    for (var colorName of Object.keys(data.brand)) {
      ...
      for (var step of Object.keys(colorData2.scale)) {
        setColorVar(brandCol, colorName + '/' + step, colorData2.scale[step]);
        report(colorName + '/' + step);
      }
    }

    var grayCol = getOrCreateCollection('컬러/그레이');
    for (var step2 of Object.keys(data.gray.scale)) {
      setColorVar(grayCol, 'Gray/' + step2, data.gray.scale[step2]);
      report('Gray/' + step2);
    }

    var semanticCol = getOrCreateCollection('컬러/시맨틱');
    ...
    for (var cn of Object.keys(data.semantic)) {
      ...
      for (var variant of Object.keys(variants)) {
        setColorVar(semanticCol, cn + '/' + (variantLabel[variant] || variant), variants[variant]);
        report(cn + '/' + variant);
      }
    }

    var spacingCol = getOrCreateCollection('여백(Spacing)');
    for (var sv of data.spacing) {
      setFloatVar(spacingCol, 'spacing-' + sv, sv);
      report('spacing-' + sv);
    }

    var radiusCol = getOrCreateCollection('둥글기(Radius)');
    for (var rv of data.radius) {
      setFloatVar(radiusCol, 'radius-' + rv, rv);
      report('radius-' + rv);
    }
...
async function drawStyleGuide(data) {
  try {
    figma.ui.postMessage({ type: 'draw-progress', msg: '폰트 로드 중...' });

    // ─ Font loading: 설치된 폰트 목록을 먼저 스캔해 정확한 이름 확인 ─
    var allFonts = await figma.listAvailableFontsAsync();

    // family → { 'Regular': true, 'Bold': true, ... } 맵 구성
    var fontMap = {};
    ...
    if (!F) {
      figma.ui.postMessage({ type: 'error', message: '사용 가능한 폰트를 찾지 못했습니다.' });
      return;
    }

    figma.ui.postMessage({ type: 'draw-progress', msg: '폰트 로드: ' + F.regular.family + ' ' + F.regular.style });

    // ─ Create Figma text styles ─
    figma.ui.postMessage({ type: 'draw-progress', msg: '텍스트 스타일 생성 중...' });
    ...
    var existTs = figma.getLocalTextStyles();
    ...
    figma.ui.postMessage({ type: 'draw-progress', msg: '보드 생성 중...' });
    ...
    // ─ Helpers (closures over F and INNER_W) ─
    ...
    function getVar(colName, vName) {
      if (!_vcache) {
        _vcache = {
          cols: figma.variables.getLocalVariableCollections(),
    ...
    function spacingSection(spacing) {
      ...
      wrap.itemSpacing = 24;
      wrap.fills = [];
      // layoutWrap may not exist in older API versions
    ...
    function buttonSection() {
      ...
      // 버튼 타입 블록 (Primary / Secondary / Ghost)
      ...
      // 브랜드 컬러 그룹 (Primary 타입 + Secondary 타입)
    ...
    function inputSection() {
      ...
      function drawInput(state, content, sz) {
        ...
        if (content === 'Placeholder') { textStr = '내용을 입력하세요'; textCol = sgRgb(ig30h); }
        else                           { textStr = '입력된 값';         textCol = { r: 0.1, g: 0.1, b: 0.15 }; }
      ...
      function drawSemanticInput(semName, semColor, sz) {
        var IH = sz.h;
        var iFS = sz.fs || (IH <= 32 ? 12 : 13);
        var messages = { Danger: '오류 메시지', Warning: '경고 메시지', Success: '성공 메시지', Info: '안내 메시지' };
        var inp = figma.createFrame();
    ...
    // ─ Assemble ─
    figma.ui.postMessage({ type: 'draw-progress', msg: '컬러 팔레트 그리는 중...' });
...
async function createComponents(data) {
  try {
    figma.ui.postMessage({ type: 'comp-progress', msg: '폰트 로드 중...' });
    ...
    if (!F) { figma.ui.postMessage({ type: 'error', message: '폰트를 찾지 못했습니다.' }); return; }

    // ─ Components 페이지 확보 ─
    var compPage = null;
    for (var pi = 0; pi < figma.root.children.length; pi++) {
      if (figma.root.children[pi].name === '📦 Components') { compPage = figma.root.children[pi]; break; }
    }
    ...
    function gv(colName, vName) {
      if (!_vc) _vc = { cols: figma.variables.getLocalVariableCollections(), vars: figma.variables.getLocalVariables() };
    ...
    // ══════════════════════════════════════════
    //  BUTTON — 브랜드별 통합 세트 (Primary+Secondary)
    //  Size × Type × State = 18 variants / 브랜드
    // ══════════════════════════════════════════
    figma.ui.postMessage({ type: 'comp-progress', msg: 'Button 컴포넌트 생성 중...' });

    var btnSets = [];
    var curX = 0;

    // ── Icon placeholder 컴포넌트 생성 ──
    ...
    // Gray 버튼 세트 (BTYPES에 Gray/Ghost 포함 시만 생성)
    if (includeGray) {
      var grayComps = [];
      BSTATES.forEach(function(stateName) {
    ...
    // ══════════════════════════════════════════
    //  INPUT — Size × Content × State
    // ══════════════════════════════════════════
    figma.ui.postMessage({ type: 'comp-progress', msg: 'Input 컴포넌트 생성 중...' });
    ...
    INPUT_SIZES.forEach(function(sz) {
      ...
      INPUT_CONTENTS.forEach(function(content) {
        INPUT_STATES.forEach(function(state) {
          ...
          if (content === 'Placeholder') { textStr = '내용을 입력하세요'; textCol = hexToFigmaColor(g30h); }
          else                           { textStr = '입력된 값';         textCol = { r: 0.1, g: 0.1, b: 0.15 }; }
    ...
    if (data.semantic) {
      ...
      var semMessages = { Danger: '오류 메시지', Warning: '경고 메시지', Success: '성공 메시지', Info: '안내 메시지' };
      INPUT_SIZES.forEach(function(sz) {
    ...
    figma.viewport.scrollAndZoomIntoView(btnSets.concat(allInputSets));
