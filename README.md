...
- **페이지 템플릿** — 생성기는 *현재 Figma 파일*에서 `[sub_page]`라는 이름의 프레임을 자동 탐지(현재 페이지 → 다른 로커 페이지 순). 외부 `.fig` 파일은 플러그인 API 제약상 직접 읽지 못함
- **샘플 `.fig` 연계** — `페이지생성기_샘플.fig`는 별도 Figma 문서이므로: ① Figma에서 해당 파일을 열어 그 안에서 플러그인을 실행(파일 내에 `[sub_page]` 템플릿 프레임이 포함되어야 함), 또는 ② 샘플의 템플릿 프레임을 작업 파일로 복사한 뒤 선택하고 **"선택 프레임을 [sub_page]로 등록"** 버튼으로 등록. CSV·MD와 달리 `.fig`는 임베드/자동 로드 대상이 아님
- 탐지된 `[sub_page]` 템플릿이 없으면 제네릭 기본 템플릿이 자동 생성됨
